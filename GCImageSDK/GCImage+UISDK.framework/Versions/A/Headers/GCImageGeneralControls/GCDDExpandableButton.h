...
//	https://github.com/ddebin/GCDDExpandableButton
//
...
#define GCDDView UIView <GCDDExpandableButtonViewSource>

@protocol GCDDExpandableButtonViewSource;

/**
 `GCDDExpandableButton` is a class designed to be used like an expandable `UIButton` ; as seen in the iOS Camera app for the "flash" button.
 */
@interface GCDDExpandableButton : UIControl
{
	...
	CGFloat		maxHeight;
	CGFloat		maxWidth;
	GCDDView		*leftTitleView;
}

@property (nonatomic,assign) UIInterfaceOrientation orientation;
@property (nonatomic,assign)	NSUInteger		foldedAfterSelectedItem;
...
/**
 Creates and returns a `GCDDExpandableButton` with specific point / left title / buttons
 @param point Upper-left corner for `GCDDExpandableButton` frame
 @param leftTitle Left title in button
 @param buttons An array of `NSString`/`UIImage`/`UIView` objects
 @return A `GCDDExpandableButton`
 */
...
/**
 All `UIView` used by GCDDExpandableButton must conform to this protocol
 */
@protocol GCDDExpandableButtonViewSource <NSObject>
